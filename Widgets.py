...
class DragButton(QPushButton):
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() == Qt.LeftButton:
            ...
            currPos = self.mapFromGlobal(self.pos())
            globalPos = event.globalPos()
            diff = globalPos - self.__mouseMovePos
            newPos = self.mapToGlobal(currPos + diff)
            self.move(newPos)
...
def returnAnimationEasingCurve(easingCurveName):
    if len(str(easingCurveName)) > 0:
        if str(easingCurveName) == "OutQuad":
            ...
            raise Exception("Unknown value'" +easingCurveName+ "' for setEasingCurve() on ", None)
########################################################################
##
########################################################################
